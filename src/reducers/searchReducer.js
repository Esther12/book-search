  ...
  switch (type) {
    case SEARCH_CONTENT:
      ...
      return {
        ...
        books: payload,
        current_page:0,
        current_books:payload.length<20?payload:payload.slice(0,20)
      };
    case RESULT_FILTER_DATE:
      let dateFille = state.books.sort(sort("first_publish_year", false, parseInt));;
      return {
        ...state,
        loading: false,
        sort:2,
        books: dateFille,
        current_page:0,
        current_books:dateFille.length<20?dateFille:dateFille.slice(0,20)
      };
    case RESULT_FILTER_NAME:
      let nameFilter = state.books.sort(sort("title", false, (a) =>  a.toUpperCase()));
      return {
        ...state,
        loading: false,
        sort:1,
        books: nameFilter,
        current_page:0,
        current_books:nameFilter.length<20?nameFilter:nameFilter.slice(0,20)
      };
    case SET_PAGE:
      return {
        ...state,
